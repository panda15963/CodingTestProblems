function maximumScore(grid: number[][]): number {
    ...
    for (let j = 1; j < n; ++j) {
        const currPick: number[] = new Array(n + 1).fill(Number.NEGATIVE_INFINITY);
        const currSkip: number[] = new Array(n + 1).fill(Number.NEGATIVE_INFINITY);
        ...
        for (let curr = 0; curr <= n; ++curr) {
            for (let prev = 0; prev <= n; ++prev) {
                if (curr > prev) {
                    ...
                    const score: number = prefix[j - 1][curr] - prefix[j - 1][prev];
                    ...
                    const score: number = prefix[j][prev] - prefix[j][curr];
