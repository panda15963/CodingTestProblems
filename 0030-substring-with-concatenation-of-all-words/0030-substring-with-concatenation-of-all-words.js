var findSubstring = function(s, words) {
    ...
    const wordLeng = words[0].length;
    const wholeLeng = words.length * wordLeng;
    ...
    for (let i = 0; i < wordLeng; i++) {
        ...
        for (
            ...
            const from = i + j * wordLeng;
            const to = (j + 1) * wordLeng + i;

            const now = s.substring(from, to);
        ...
        for (let j = words.length; j < createCnt; j++) {
            // 맨 앞 단어 제거
            const deleteKey = s.substring(
                deleteIndex,
                deleteIndex + wordLeng
            );

            const newVal = compareMap.get(deleteKey) - 1;
