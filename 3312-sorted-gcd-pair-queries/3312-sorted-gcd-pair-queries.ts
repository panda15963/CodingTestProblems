function gcdValues(nums: number[], queries: number[]): number[] {
    ...
    const frequencyCount: number[] = new Array(maxValue + 1).fill(0);
    ...
    const gcdPairCount: number[] = new Array(maxValue + 1).fill(0);
    ...
    for (let gcd = maxValue; gcd >= 1; gcd--) {
        ...
        gcdPairCount[gcd] += Math.floor(multiplesCount * (multiplesCount - 1) / 2);
    }
...
function upperBound(array: number[], target: number): number {
    ...
    let firstTrueIndex = array.length;  // Default: not found
