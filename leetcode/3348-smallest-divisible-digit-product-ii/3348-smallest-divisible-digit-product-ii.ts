...
type DigitFactors = Partial<PrimeCount>;

const FACTOR_COUNTS: Record<number, DigitFactors> = {
  0: {},
  1: {},
  2: { 2: 1 },
  3: { 3: 1 },
...
function smallestNumber(num: string, t: number | bigint): string {
  let target = BigInt(t);

  const [primeCount, isDivisible] = getPrimeCountT(target);
...
function getPrimeCountT(t: bigint): [PrimeCount, boolean] {
  ...
  for (const prime of [2, 3, 5, 7] as const) {
...
function getPrimeCountString(num: string): PrimeCount {
  const count: PrimeCount = {
    2: 0,
    3: 0,
    5: 0,
...
function getFactorCount(count: PrimeCount): FactorCount {
...
function construct(factors: FactorCount): string {
  let result = "";

  for (let digit = 2; digit <= 9; digit++) {
    result += String(digit).repeat(factors[digit as keyof FactorCount]);
  }

  return result;
}

function isSubset(a: PrimeCount, b: PrimeCount): boolean {
  for (const prime of [2, 3, 5, 7] as const) {
    if (b[prime] < a[prime]) {
      return false;
    }
...
function subtract(
  ...
  const result: PrimeCount = { ...a };

  for (const prime of [2, 3, 5, 7] as const) {
    result[prime] = Math.max(
      0,
      result[prime] - (b[prime] ?? 0)
    );
  }
